"""
bench_decode.py — Relay decode vs json.loads throughput benchmark.

Measures decoding throughput for small (~100B), medium (~1KB), and large (~10KB)
...
# ---------------------------------------------------------------------------
# Payload fixtures (identical to bench_encode.py for cross-benchmark consistency)
# ---------------------------------------------------------------------------
...
def bench_relay_decode(relay_bytes: bytes, iterations: int) -> float:
    """Return microseconds per operation (us/op)."""
    elapsed = timeit.timeit(
        stmt=lambda: relay.decode(relay_bytes),
        number=iterations,
    )
    return (elapsed / iterations) * 1e6


def bench_json_decode(json_str: str, iterations: int) -> float:
    """Return microseconds per operation (us/op)."""
    elapsed = timeit.timeit(
        stmt=lambda: json.loads(json_str),
        number=iterations,
    )
    return (elapsed / iterations) * 1e6
...
def main() -> None:
    ...
    # Pre-encode Relay bytes and JSON strings so decoding benchmarks are clean
    small_relay = relay.encode(SMALL_DICT, small_schema)
    medium_relay = relay.encode(MEDIUM_DICT, medium_schema)
    large_relay = relay.encode(LARGE_DICT, large_schema)
    ...
    cases = [
        ("small  (~100 B)", small_relay, small_json),
        ("medium (~1 KB) ", medium_relay, medium_json),
        ("large  (~10 KB)", large_relay, large_json),
    ]

    target_ratio = 1.5  # relay decode must be >= 1.5x faster than json.loads

    print()
    print("=" * 78)
    print("  Relay decode vs json.loads — decode throughput benchmark")
    ...
    all_passed = True
    for label, relay_bytes, json_str in cases:
        relay_us = bench_relay_decode(relay_bytes, ITERATIONS)
        json_us = bench_json_decode(json_str, ITERATIONS)
